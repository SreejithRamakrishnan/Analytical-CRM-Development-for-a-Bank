...
FROM (
	SELECT Surname, YEAR(Bank_DOJ) AS `YEAR`, EstimatedSalary, 
        DENSE_RANK() OVER(PARTITION BY YEAR(Bank_DOJ) ORDER BY EstimatedSalary DESC) AS rn 
	FROM customer_info
...
SELECT AVG(NumofProducts) AS Num_of_products 
FROM bank_churn
...
SELECT GeographyLocation, num_Active_customers 
FROM 
	(SELECT g.GeographyLocation, 
	COUNT(g.GeographyLocation) AS num_Active_customers,
...
# Examine the trend of customer joining over time and identify any seasonal patterns (yearly or monthly). 
...
SELECT b.*, e.ExitCategory 
FROM bank_churn b, exit_customer e
...
SELECT age_bracket, 
	...
	Average_salary, 
	Average_CreditScore, 
	Average_NumofProducts, 
	churn_rate, 
	retention_rate, 
	credit_card_adop_rate, 
	Active_members, 
	Inactive_members, 
	Average_Tenure 
FROM (SELECT 
	CASE 
		...
		WHEN c.Age BETWEEN 30 AND 50 THEN '30-50'
		WHEN c.Age > 50 THEN '50+' End as age_bracket, 
	COUNT(CASE WHEN b.Has_creditCard = 1 THEN b.CustomerID END) 
	AS num_customers,
...
#In the “Bank_Churn” table how can you modify the name of “HasCrCard” column to “Has_creditcard”?
